...
public class InetEndpoint {
    ...
    InetEndpoint(@Nullable final String endpoint) {
        if (endpoint.indexOf('/') != -1 || endpoint.indexOf('?') != -1 || endpoint.indexOf('#') != -1)
            throw new IllegalArgumentException(Application.Companion.get().getString(R.string.tunnel_error_forbidden_endpoint_chars));
        final URI uri;
        try {
    ...
    String getResolvedEndpoint() throws UnknownHostException {
